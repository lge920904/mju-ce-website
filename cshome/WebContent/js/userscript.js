...
/*
 * PHP passes the form structure to JS using 'fm_register_form_item' and
 * 'fm_register_form'
 */
...
// returns the 'registered' information about the form element, essentially the
// DB entry
...
// does nothing for now; appropriate script should be attached to the button to
// begin with
...
function fm_submit_onclick(formID) {
	if (!fm_check_required_items(formID))
		return false;
	if (!fm_check_text_validation(formID))
		return false;
	return true;
}

function fm_submit_onclick_ajax(formID, formSlug) {
	if (fm_registered_forms[formID])
		return false;
	...
	for ( var x = 0; x < fm_registered_form_items.length; x++) {
		if (fm_registered_form_items[x].formID == formID) {
			eval("temp = " + fm_registered_form_items[x].getter_script + "(\'"
					+ formID + "\', \'"
					+ fm_registered_form_items[x].unique_name + "\');");
...
function fm_item_validation_satisfied(itemDef) {
	if (itemDef.validation_callback != "") {
		eval("temp = " + itemDef.validation_callback + "('" + itemDef.formID
				+ "', '" + itemDef.unique_name + "', '"
				+ itemDef.validation_type + "');");
		return temp;
	}
	return true;
}
...
function fm_item_required_satisfied(itemDef) {
	if (itemDef.required == 1 && itemDef.required_callback != "") {
		eval("temp = " + itemDef.required_callback + "('" + itemDef.formID
				+ "', '" + itemDef.unique_name + "');");
		return temp;
	}
	return true;
...
function fm_set_required(itemID, req) {
	for ( var x = 0; x < fm_registered_form_items.length; x++) {
		...
		if (temp.unique_name == itemID) {
			...
			tempID = 'fm-item-'
					+ (temp.nickname != "" ? temp.nickname : temp.unique_name);
...
function fm_custom_list_required_validator(formID, itemID) {
	var listType = document.getElementById('fm-form-' + formID)[itemID
			+ '-list-style'].value;
	switch (listType) {
	case "radio":
...
function fm_checkbox_list_required_validator(formID, itemID) {
	...
	for ( var x = 0; x < count; x++) {
		if (document.getElementById('fm-form-' + formID)[itemID + '-' + x].checked)
			return true;
	}
	return false;
}