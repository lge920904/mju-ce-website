...
public class UserInfoServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (action.equals(ACTION.LOGIN.getAction())) {
			...
			if(user == null){
				session.setAttribute("loginError", "입력하신 정보가 없습니다.");
				out.println("yaho");
			}else if(user.getPassword().equals(password) && user.getActive() == 2014){
				...
				session.setAttribute("loginError", "계정이 비활성화되었습니다.");
			}
